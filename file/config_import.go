...
// parseConfigWalkYaml 全盘解析配置文件，yaml格式, 将配置信息解析到allConfig中
func paraseConfigWalkYaml(loadPath string) (*allConfig, error) {
...
func ConfigImportYaml(loadPath string) error {
	...
	all, err := paraseConfigWalkYaml(loadPath)
	if err != nil {
		return err
	}
...
func buildFlow(all *allConfig, fp config.KisFlowFunctionParam, newFlow *flow.KisFlow, flowName string) error {
	// 加载当前Flow依赖的Function
